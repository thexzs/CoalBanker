...
public class GoToCoals extends Task<ClientContext>{
	...
	public GoToCoals(ClientContext ctx) {
		super(ctx);
	}
	...
	public boolean activate() {
		...
		&& !(ctx.backpack.select().id(oreID).count() > 0)
		&& !(ctx.players.local().animation() == 625);
	}

	@Override
	public void execute() {
		CoalBanker.stateofplayer = "Going to Coals";
		CoalBanker.statesignal = "goingtoCoals";
		System.out.println("[xzs] " + CoalBanker.statesignal);
		pathToCoals = new TilePath(ctx, PATH);
